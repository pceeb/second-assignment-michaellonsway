...
2. What is the first thing that you do once you login?  What is the command? The first thing to do after logging in is getting a compute node by using the command qrsh
...
The ls -l command will use a long listing format and ls -l -h is a long listing format that is in human readable format
...
It will display things by time of last change but will also list their sub-directories and sub-sub directories.
...
$ cd /     This changes the directory to root directory
...
$ cd home     This changes the directory to a directory called home.
$ cd ~/data/..     This changes the directory to home. Amanda would use this to navigate to her home directory.
$ cd     This changes the working directory. Amanda would have to put a directory after cd to navigate to her home directory.
...
```
  ...
  b. `ls -r -F`     This command will result in the following output (b)

  c. `ls -r -F /Users/backup`     This command will result in the following output (c)
...
```
  ...
  c. The first three lines and the last two lines of animals.txt This answer corresponds to the file animals-subset.txt (c)
...
After creating and saving this file you realize you misspelled the filename! You want to correct the mistake, which of the following commands could you use to do so?
  ...
  b. `mv statstics.txt statistics.txt` This command would work (b)
...
b. `recombine` The output of the closing command is recombine (b)
...
This will remove quotations.txt but will prompt you if you want to remove it. We want this protection in order to prevent us from accidentally removing an important file.

5. In the example below, what does cp do when given several filenames and a directory name? 
Cp will copy multiple files and move them to the directory that is listed
```
$ mkdir backup
...
It will give an error because a directory isn't listed
```
$ ls -F
```
...
Uniq only removes adjacent duplicated lines in order to reduce memory usage. We could sort all the lines alphabetically by using sort and then use uniq to remove all duplicated lines. For this example we would use sort salmon.txt and then uniq salmon.txt.
...
What other command(s) could be added to this in a pipeline to find out what animals the file contains (without any duplicates in their names)? We could use the command sort and then use the command uniq in order to delete adjacent duplicate files so then we would find out what animals the file contains. cut -d, -f 2 animals.txt | sort | uniq
...
cut -d, -f 2 animals.txt | sort | uniq -c This command would be used to produce a table
