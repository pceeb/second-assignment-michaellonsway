...
1. What is the procedure for logging into Hoffman2? Log in using your username (username@hoffman2.idre.ucla.edu) and password
2. What is the first thing that you do once you login?  What is the command? Getting a compute node using the command qrsh
...
3. what is ./?     It is the current directory that we are in
...
````
$ cd .     This keeps us in the current directory we are in
$ cd /     This changes the directory to /
$ cd /home/amanda     This changes the directory to amanda, which is within the home directory
$ cd ../..     This changes the directory to the directory that is 2 directories previous the current one
$ cd ~     This changes the directory to home. Amanda would use this to navigate to her home directory.
...
1. Using the filesystem diagram below, if pwd displays `/Users/thing`, what will `ls -F ../backup` display?
  ...
  d. `original/ pnas_final/ pnas_sub/`     It will display d.
...
```
  ...
  b. `ls -r -F`     This command will result in the following output

  c. `ls -r -F /Users/backup`     This command will result in the following output
...
$ touch my_file.txt
```
 What did the touch command do? When you look at your current directory using the GUI file explorer, does the file show up? The touch command creates a file and the file does show up in the current directory.

 Use 'ls -l' to inspect the files. How large is my_file.txt? The file is 0 kilobytes

 When might you want to create a file this way? In order to create an empty file

 Can you use touch to edit my_file.txt? No, you would have to use nano
...
Using > will overwrite information and using >> will not overwrite and will place new information.
...
```
$ head -n 3 animals.txt > animals-subset.txt  a: The first three lines of animals.txt
```

```
$ tail -n 2 animals.txt >> animals-subset.txt b: The last two lines of animals.txt
```
...
After creating and saving this file you realize you misspelled the filename! You want to correct the mistake, which of the following commands could you use to do so?
  ...
  b. `mv statstics.txt statistics.txt` This command would work
...
b. `recombine` The output of the closing command is recombine
c. `proteins.dat recombine`
d. `proteins-saved.dat`


4. What happens when we execute `rm -i thesis_backup/quotations.txt`? Why would we want this protection when using rm? This will remove quotations.txt but will prompt you if you want to remove it. We want this protection in order to prevent us from accidentally removing an important file.

5. In the example below, what does cp do when given several filenames and a directory name? Cp will copy multiple files and move them to the directory that is listed
```
$ mkdir backup
$ cp amino-acids.txt animals.txt backup/
```

6. In the example below, what does cp do when given three or more file names? It will copy multiple files at once
```
$ ls -F
```
...
$ mkdir 2016-05-20                     This command would make a directory called 2016-05-20
$ cd 2016-05-20                        This would move you to the 2016-05-20 directory
$ mkdir data                           This would make a directory called data
$ cd data                              4*
$ mkdir raw processed                  5*
$ mkdir 2016-05-20/data/raw            This would make a directory called raw within the data directory
$ mkdir 2016-05-20/data/processed      This would make a directory called processed within the data directory
$ mkdir 2016-05-20                     1*
$ cd 2016-05-20                        2*
$ mkdir data                           3*
$ mkdir raw processed                  This would make the directories raw and processed
```
---
...
Explain why -n has this effect. The -n sorts the files numerically from lowest number to highest
...
steelhead
```
Why do you think uniq only removes adjacent duplicated lines? (Hint: think about very large data sets.) What other command could you combine with it in a pipe to remove all duplicated lines? Uniq only removes adjacent duplicated lines in order to reduce memory usage. We could sort all the lines alphabetically by using sort and then use uniq to remove all duplicated lines. For this example we would use sort salmon.txt and then uniq salmon.txt.
...
What text passes through each of the pipes and the final redirect in the pipeline below? The first 5 and last 3 texts would pass through so all of the text present in animals.txt would pass through each of the pipes and the final redirect and they would be sorted alphabetically from z-a.
```
...
What other command(s) could be added to this in a pipeline to find out what animals the file contains (without any duplicates in their names)? We could use the command sort and then use the command uniq in order to delete adjacent duplicate files so then we would find out what animals the file contains.
...
```
grep {deer, rabbit, raccoon, deer, fox, bear} animals.txt | wc -l This command would be used to produce a table
...
Can you match the same set of files with basic wildcard expressions that do not use the [ ] syntax? Hint: You may need more than one expression. Yes, by using \*A.txt and \*B.txt

The expression that you found and the expression from the lesson match the same set of files in this example. What is the small difference between the outputs? The expression from the lesson gives the A.txt and B.txt at the same time while the expression that I found gives the A.txt and B.txt separately

Under what circumstances would your new expression produce an error message where the original one would not? If you run both expressions together on the same line you will produce an error message
